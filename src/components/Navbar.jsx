...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] w-full px-[4vw] py-4 bg-zinc-300/1 backdrop-blur-[50px] text-[#000] flex justify-between items-center transition-transform duration-300 ${
      showNavbar ? 'translate-y-0' : '-translate-y-full'
        ...
        <div className='links flex gap-6'>
            {["Services", "Our Work", "About Us", "Insights", "Contact us"].map((item, index) => (
                <a key={index} className={`text-[1.2vw] font-md tracking-tight ${index === 4 && "ml-40"}`}>{item}</a>
            ))}
        </div>
